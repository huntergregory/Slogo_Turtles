...
##### What about your API/design is intended to be flexible?
The ability to change anything in the turtleView in the backend and simply pass a immutable version of the turtleView as a response every time, and an inheritance hiearchy of Result classes allows for different kinds of commands and responses to be run. For example, there is a ErrorResult class which can be sent. 
##### How is your API/design encapsulating your implementation decisions?
...
##### What exceptions (error cases) might occur in your part and how will you handle them (or not, by throwing)?
All errors that are encountered from commands will be thrown back to the user via the ErrorResult response from the backend. This can be a bad syntax command, uninitalized variable or command, divide by 0/arithemetic error, or operation errors such as trying to move the turtleView out of bounds. 
...
##### What feature/design problem are you most worried about working on?
I am most worried to work on the TurtleViewer, and displaying movement and lines of the turtleView when it could be a non-linear path.
...
4. Clicking on the turtleView and changing the image via an image browser
5. Making animations for the turtleView using an AnimationsGenerator depending on the kind of path generated by the turtleView.
